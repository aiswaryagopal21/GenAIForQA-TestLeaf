...
class ChatUI {
    ...
    initialize() {
        // Test Data Generator checkbox logic
        const testDataCheckbox = document.getElementById('javaGenModeTestData');
        const inspectorButton = this.inspectorButton;
        const chatInput = this.inputField;
        // Add number of records input
        let numRecordsInput = document.getElementById('testDataNumRecords');
        if (!numRecordsInput) {
            numRecordsInput = document.createElement('input');
            numRecordsInput.type = 'number';
            numRecordsInput.id = 'testDataNumRecords';
            numRecordsInput.min = '1';
            numRecordsInput.max = '100';
            numRecordsInput.value = '1';
            numRecordsInput.placeholder = 'No. of records';
            numRecordsInput.style.marginLeft = '8px';
            numRecordsInput.style.width = '120px';
            chatInput.parentNode.insertBefore(numRecordsInput, chatInput.nextSibling);
        }

        function updateTestDataUI() {
            if (testDataCheckbox && testDataCheckbox.checked) {
                inspectorButton.disabled = true;
                inspectorButton.classList.add('disabled');
                numRecordsInput.style.display = '';
            } else {
                inspectorButton.disabled = false;
                inspectorButton.classList.remove('disabled');
                numRecordsInput.style.display = 'none';
            }
        }
        if (testDataCheckbox) {
            testDataCheckbox.addEventListener('change', updateTestDataUI);
            updateTestDataUI();
        }
        // Reset chat
        if (this.resetButton) {
            this.resetButton.addEventListener('click', () => {
                // Remove all chat messages (including test data results)
                this.messagesContainer.innerHTML = '';
                this.addMessage(INITIAL_SYSTEM_MESSAGE, 'system');
                this.selectedDomContent = null;
                this.generatedCode = '';
                ...
                // Also reset test data generator fields if present
                if (numRecordsInput) numRecordsInput.value = 1;
    ...
    async sendMessage() {
        ...
        if (this.selectedProvider === 'groq') apiRef = this.groqAPI;
                ...
                // If Test Data Generator is checked, skip DOM requirement and generate test data
                const testDataCheckbox = document.getElementById('javaGenModeTestData');
                const numRecordsInput = document.getElementById('testDataNumRecords');
                                if (testDataCheckbox && testDataCheckbox.checked) {
                                        const typeRaw = this.inputField.value.trim().toLowerCase();
                                        let count = parseInt(numRecordsInput.value, 10);
                                        if (isNaN(count) || count < 1) count = 1;
                                        if (count > 100) count = 100;
                                        // Support comma-separated types
                                        const types = typeRaw.split(',').map(t => t.trim()).filter(Boolean);
                                        // Sample data pools
                                        const names = ['Aishu', 'John', 'Priya', 'Kumar', 'Sneha', 'Rahul', 'Meena', 'Vikram', 'Sara', 'Arjun'];
                                        const emails = ['aishu', 'john', 'priya', 'kumar', 'sneha', 'rahul', 'meena', 'vikram', 'sara', 'arjun'];
                                        function randomPhone() { return String(Math.floor(7000000000 + Math.random()*1000000000)); }
                                        function randomEmail(name) { return name.toLowerCase() + Math.floor(Math.random()*100) + '@example.com'; }
                                        function randomAddress() { return Math.floor(Math.random()*9999) + ' Main St, City'; }
                                        ...
                                        for (let i = 0; i < count; i++) {
                                                ...
                                                for (const type of types) {
                                                    if (type === 'email' || type === 'email address') {
                                                        // Use a random name for email
                                                        const name = names[Math.floor(Math.random()*names.length)];
                                                        row.push(randomEmail(name));
                                                    } else if (type === 'name' || type === 'full name') {
                                                        row.push(names[Math.floor(Math.random()*names.length)]);
                                                    } else if ([
                                                        'phone','phone number','phonenumber','mobile','mobile number','mobilenumber'
                                                    ].includes(type)) {
                                                        row.push(randomPhone());
                                                    } else if (type === 'address') {
                                                        row.push(randomAddress());
                                                    } else {
                                                        row.push('SampleData_' + Math.random().toString(36).substring(2,10));
                                                    }
                                                }
                                                results.push(row);
                                        }
                                        // Render as table if multiple types, else as list
                                        const msgDiv = document.createElement('div');
                                        msgDiv.className = 'chat-message assistant-message';
                                        const mdDiv = document.createElement('div');
                                        mdDiv.className = 'markdown-content';
                                        let html = `<strong>Test Data (${types.join(', ')}, ${count} records):</strong><br>`;
                                        if (types.length > 1) {
                                            html += `<table style='margin: 0 auto; border-collapse: collapse;'><thead><tr>${types.map(t => `<th style='border:1px solid #ccc;padding:4px 8px;'>${t}</th>`).join('')}</tr></thead><tbody>`;
                                            html += results.map(row => `<tr>${row.map(cell => `<td style='border:1px solid #ccc;padding:4px 8px;'>${cell}</td>`).join('')}</tr>`).join('');
                                            html += '</tbody></table>';
                                        } else {
                                            html += `<ul style='text-align:left;max-width:400px;margin:0 auto;'>${results.map(row => `<li>${row[0]}</li>`).join('')}</ul>`;
                                        }
                                        mdDiv.innerHTML = html;
                                        msgDiv.appendChild(mdDiv);
                                        // Copy button
                                        const copyBtn = document.createElement('button');
                                        copyBtn.innerHTML = '<i class="fas fa-copy"></i> Copy';
                                        copyBtn.style.margin = '8px 8px 8px 0';
                                        copyBtn.onclick = () => {
                                                let text;
                                                if (types.length > 1) {
                                                    text = results.map(row => row.join(', ')).join('\n');
                                                } else {
                                                    text = results.map(row => row[0]).join('\n');
                                                }
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked && isTestDataChecked) {
            // All three selected - generate all outputs
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('CUCUMBER_WITH_SELENIUM_JAVA_STEPS');
                promptKeys.push('TEST_DATA_GENERATOR');
            } else {
            ...
            if (isFeatureChecked && isPageChecked) {
                ...
                if (this.isJavaSelenium(lang, eng)) {
                    promptKeys.push('SELENIUM_JAVA_PAGE_ONLY');
                } else if (this.isPlaywrightTS(lang, eng)) {
    ...
    isPlaywrightTS(language, engine) {
